...
function trials.f_inittrials()
	trials = {
		...
		starttick = tickcount(),
		elapsedtime = 0,
		trial = f_deepCopy(start.f_getCharData(start.p[1].t_selected[1].ref).trials),
		displaytimers = {
			totaltimer = true,
			trialtimer = true,
		},
...
function trials.f_trialsBuilder()
	...
	if main.debugLog then main.f_printTable(trials, "debug/t_trials.txt") end
end
...
for row = 1, #main.t_selChars, 1 do
	...
	if  main.t_selChars[row].def ~= nil and main.t_selChars[row].trialsdef ~= "" then
		...
		main.t_selChars[row].trials = trial
	end
end